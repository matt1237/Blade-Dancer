...
func _build_charged_guard_tab(tabs: TabContainer) -> void:
	...
	note.text = "Drive the hand strongly toward the pommel during the counter-swing to acquire Guard State (yellow ring). Charged Guard State begins when the optional blue state completes its confirmation hold. Tune the folded pose, phase window, and charge-rate boosts below. A deliberate authored flick releases the lock; so does a completed gesture, which discharges the guard as an attack instead of merely dropping it."
	note.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	box.add_child(note)
	_create_contact_slider(box, "charged_guard_enabled", "Charged Guard Enabled", 0.0, 1.0, 1.0, "", _form_three_feel_tip("Opt in to acquiring a counter-swing guard; off preserves normal combat exactly.", "Guard recognition and lock are disabled.", "Pommel-drive can acquire and charge guard.", "Tune the gesture below; no extra button is used."))
	...
	_create_contact_slider(box, "charged_guard_hold_duration", "Guard Charge Time", 0.05, 0.60, 0.01, " s", _form_three_feel_tip("Time required holding the folded guard to lock it.", "Locks quickly after a valid fold.", "Requires a longer, steadier guard hold.", "Charge-rate boosts below can shorten the real time when their cues are present."))
	_create_contact_slider(box, "charged_guard_awaken_duration", "Charged Position Confirm Time", 0.20, 2.00, 0.05, " s", _form_three_feel_tip("Time to maintain a locked guard before its hand position becomes fully charged and blue.", "The blue charged state arrives quickly.", "The hand must remain steady longer before the charged shimmer appears.", "Default is one second. Once fully charged, the state persists until a medium/fast authored flick breaks it."))
	_create_contact_slider(box, "charged_guard_break_speed", "Guard Break Speed Threshold", 100.0, 2000.0, 25.0, " px/s", _form_three_feel_tip("How fast the aim must sweep sideways to break a fully charged guard, measured while the hand can still follow the cursor.", "A gentler sideways sweep breaks it, so the charged state is easier to leave.", "The guard shrugs off quick sweeps and only a hard sideways yank breaks it.", "Default 600 px/s. Only sideways aim movement counts, so pushing the cursor straight out to max hand range never breaks the guard however fast it is shoved. Once the cursor is past max hand range the guard counts only the share of a sweep that could actually move the hand, so the further out you reach the harder you must swing: a quick sweep that would break the guard up close no longer breaks it at range, and reaching out can no longer drop the state by accident. This is the only thing that sets how hard the guard is to break; it is not the only way to leave it, since a completed gesture discharges the guard instead. The locked hand's reposition speed is a separate fixed value."))
	_create_contact_slider(box, "charged_guard_acquisition_window", "Pommel Acquisition Phase Window", 0.20, 0.95, 0.01, "", _form_three_feel_tip("How far from the metronome reversal the pommel gesture may begin; this is the abs(cos(phase)) boundary.", "Narrow window centered on the reversal.", "More of the counter-swing phase can qualify.", "This only widens timing; the strong pommel-alignment gesture remains required."))
	_create_contact_slider(box, "charged_guard_pommel_alignment", "Pommel Direction Precision", 0.50, 0.99, 0.01, "", _form_three_feel_tip("Minimum fraction of authored aim movement that must point along the pommel axis.", "More diagonal pulls may qualify.", "Requires a nearly straight pull opposite the blade tip.", "This is strict acquisition, not a charge-rate boost."))
	_create_contact_slider(box, "charged_guard_pommel_speed", "Minimum Pommel-Drive Speed", 20.0, 300.0, 5.0, " px/s", _form_three_feel_tip("Minimum authored hand speed for deliberate pommel drive and recent-motion credit.", "Slower pulls can qualify and count as recent movement.", "Requires a faster, more deliberate drive.", "Keep low enough for your input device to reach consistently."))
	_create_contact_slider(box, "charged_guard_pommel_travel", "Pommel-Drive Travel Required", 2.0, 40.0, 1.0, " px", _form_three_feel_tip("Aligned pommel-axis travel needed before Guard acquisition.", "A short pull can establish intent.", "Requires a longer sustained pull.", "This combines with minimum drive time; both must be met."))
	_create_contact_slider(box, "charged_guard_pommel_intent_time", "Pommel-Drive Time Required", 0.02, 0.20, 0.01, " s", _form_three_feel_tip("Minimum duration of continuous qualifying pommel movement before acquisition.", "A brief, crisp pull is enough.", "Requires the pull to stay aligned longer.", "The phase window must remain valid while the gesture is accumulated."))
