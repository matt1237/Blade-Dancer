...
func _build_charged_guard_tab(tabs: TabContainer) -> void:
	...
	note.text = "Acquire Guard State by driving the pommel back against the sword's metronome swing: as the blade sweeps one way, drive the hilt the other, so the swing cancels out and the blade holds still in a guard shape. Catching the turn of the stroke -- the moment the blade reaches full extension -- charges it faster, but never decides whether the guard takes. Keeping the drive steady is what holds the guard: drift out of that shape and it slips away. An amber ring fills around the hand as the guard builds, a closing amber ring is the confirmation hold, and the hand only turns blue once the charged state is actually up -- nothing is blue until then. A hard sideways flick breaks a held guard, the Guard Hold Limit runs it out, and a completed gesture discharges it as an attack."
	...
	_create_contact_slider(box, "charged_guard_acquisition_window", "Stroke-Turn Charge Bonus Window", 0.20, 0.95, 0.01, "", _form_three_feel_tip("How much of the stroke around each turn counts as catching the blade at full extension; a wider window earns the charge-speed bonus more often.", "The charge bonus only lands in a narrow moment at each end of the stroke.", "Most of the stroke counts as catching the turn.", "A reward, never a permission: nothing about whether a guard takes depends on being in this window. It only decides how often the drive charges faster."))
	...
	_create_contact_slider(box, "charged_guard_pommel_travel", "Pommel Drive Travel Required", 2.0, 40.0, 1.0, " px", _form_three_feel_tip("How much counter-drive travel must bank before a guard latches.", "A short drive sets the guard.", "A long sustained drive is required.", "The bank is capped at this bar, bleeds off in about a fifth of a second once the drive lapses, and is never drawn, so a guard only appears on screen once it has been earned. Works with the time requirement below."))
