...
- Grapple Yo-yo: `res://scripts/grapple_controller.gd` + `res://scripts/chakram.gd` — the active mechanic is direct flying-Chakram attachment, tuned slack take-up, tangential preservation, hand redirection, hang, and reel return. Enemy/terrain boundary wrapping and the static tether pivot are shelved and disabled by default; their retained solver is dormant. GP2 slot 2 has both wrap switches set false through the canonical save authority. Audit findings and remaining gameplay limits: `res://Ziva Help/GRAPPLE_YOYO_PARKING_LOT.md`.
