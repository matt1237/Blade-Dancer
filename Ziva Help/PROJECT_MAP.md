...
- Grapple Yo-yo: `res://scripts/grapple_controller.gd` + `res://scripts/chakram.gd` — a grappled flying Chakram receives full line, progressively loses outward radial motion near maximum extension, preserves tangent into an orbit/hang, then hands off to the existing reel. One static obstruction/wall corner may become a local pivot with path-length accounting, rope rendering, and clear-path unwrapping. All 33 Grapple/Yo-yo tuners persist through the one canonical registry; GP2 is the current source-of-truth package. Design/follow-ups: `res://Ziva Help/GRAPPLE_YOYO_PARKING_LOT.md`.
