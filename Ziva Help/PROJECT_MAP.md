...
- Grapple Yo-yo: `res://scripts/grapple_controller.gd` + `res://scripts/chakram.gd` — attachment uses current path + initial slack, not a full-range grant. Live take-up preserves orbit/hang; existing reel shortens available rope. Full boundary mode tracks one enemy circle or axis-aligned terrain rectangle with live supporting contacts and signed geometric winding; fallback is a single static pivot. Rendering consumes the same boundary path. GP2 uses the canonical tuner registry/save path with both wrap switches explicitly true. Enemy wrapping is required, NOT parked. Bounded repair findings, helper verification and remaining gameplay limits: `res://Ziva Help/GRAPPLE_YOYO_PARKING_LOT.md`.
