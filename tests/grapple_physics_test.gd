...
func test_grounded_chakram_grapple_relaunches_into_yoyo_control() -> void:
	var player: Player = preload("res://scenes/player.tscn").instantiate() as Player
	add_child(player)
	...
	chakram.velocity = Vector2.ZERO
	chakram.become_airborne_for_grapple()
	assert(not chakram.grounded, "A grappled downed Chakram must become an ordinary airborne grapple target.")
	chakram.on_grapple_attached()
	assert(chakram.grapple_attached, "The relaunched Chakram must enter normal grapple/Yo-yo control instead of auto-returning.")
	assert(chakram.velocity == Vector2.ZERO, "Relaunch must begin from rest so the grapple hand supplies the throw.")
	var grapple_source: String = FileAccess.get_file_as_string("res://scripts/grapple_controller.gd")
	assert(not grapple_source.contains("and not (node as Chakram).grounded"), "Grounded Chakrams must remain valid grapple targets.")
	assert(grapple_source.contains("become_airborne_for_grapple()"), "Hook arrival must relaunch a downed Chakram before normal attachment.")
	chakram.free()
	player.free()
